...
# ── CSS injected once (not inside a function that re-runs) ──────────────────
CHATBOT_CSS = """
<style>
/* ═══════════════════════════════════════════════════════
   CHATBOT FLOATING PANEL — bottom-right corner
   Pure fixed-position approach, no sidebar dependency.
═══════════════════════════════════════════════════════ */
...
/* ── FAB pill button (visual only, pointer-events:none) ── */
.cb-fab-pill {
    position: fixed;
    bottom: 24px;
    right: 24px;
...
/* ── Fix the FAB Streamlit button to bottom-right ── */
/* Target the block containing the FAB button using the marker div */
div[data-testid="stVerticalBlock"]:has(#cb-fab-marker) {
...
div[data-testid="stVerticalBlock"]:has(#cb-fab-marker) > div {
    background: transparent !important;
}
div[data-testid="stVerticalBlock"]:has(#cb-fab-marker) .stButton > button {
    opacity: 0 !important;
    width: 185px !important;
...
/* Also hide the visual pill markdown in this container */
div[data-testid="stVerticalBlock"]:has(#cb-fab-marker) [data-testid="stMarkdownContainer"] {
    display: none !important;
}

/* ── Fix the chatbot panel container ── */
div[data-testid="stVerticalBlock"]:has(#cb-panel-marker) {
    position: fixed !important;
    ...
    box-shadow: 0 32px 80px rgba(0,0,0,0.22), 0 8px 24px rgba(233,30,140,0.10) !important;
    border: 1.5px solid #F8BBD9 !important;
    z-index: 9998 !important;
    scrollbar-width: thin !important;
    scrollbar-color: #F8BBD9 transparent !important;
}
div[data-testid="stVerticalBlock"]:has(#cb-panel-marker)::-webkit-scrollbar { width: 3px; }
div[data-testid="stVerticalBlock"]:has(#cb-panel-marker)::-webkit-scrollbar-thumb {
...
.diag-result-card {
    ...
    border: 1px solid #F0E6F0; overflow: hidden;
    margin: 12px 0; box-shadow: 0 4px 16px rgba(0,0,0,0.06);
}
...
.step-num {
    width: 20px; height: 20px; background: #FCE4EC;
    border-radius: 50%; display: flex; align-items: center;
    justify-content: center; font-size: 10px; font-weight: 700;
    color: #E91E8C; flex-shrink: 0; margin-top: 2px;
}
.remedy-card {
...
/* ── Buttons inside chatbot panel ── */
div[data-testid="stVerticalBlock"]:has(#cb-panel-marker) .stButton > button[kind="primary"] {
    background: linear-gradient(135deg, #E91E8C, #AD1457) !important;
    border: none !important; color: white !important;
}
div[data-testid="stVerticalBlock"]:has(#cb-panel-marker) .stButton > button[kind="primary"]:hover {
    background: linear-gradient(135deg, #AD1457, #880E4F) !important;
    transform: translateY(-1px) !important;
}
div[data-testid="stVerticalBlock"]:has(#cb-panel-marker) .stButton > button {
    border-radius: 50px !important;
    font-family: 'DM Sans', sans-serif !important;
    font-weight: 600 !important;
}
/* Padding for panel body content */
div[data-testid="stVerticalBlock"]:has(#cb-panel-marker) > div[data-testid="element-container"]:not(:first-child):not(:nth-child(2)) {
...
def render_chatbot():
    """
    Renders FAB toggle + chatbot panel. Uses CSS :has(#marker) to fix
    the containing stVerticalBlock to the viewport bottom-right corner.
    """

    # ── Inject CSS ───────────────────────────────────────────────────────────
    st.markdown(CHATBOT_CSS, unsafe_allow_html=True)

    # ── Ensure all required session keys ────────────────────────────────────
    if "chatbot_open"         not in st.session_state: st.session_state.chatbot_open       = False
    if "chatbot_mode"         not in st.session_state: st.session_state.chatbot_mode       = None
    ...
    if SUGGESTION_RESULTS_KEY not in st.session_state: st.session_state[SUGGESTION_RESULTS_KEY] = None

    # ── FAB: visual pill + invisible real button ─────────────────────────────
    # cb-fab-marker causes CSS to position this block fixed bottom-right.
    if not st.session_state.chatbot_open:
        pill_html = """<div id="cb-fab-marker"></div>
        <div class="cb-fab-pill">
            <span style="font-size:18px;">🌿</span> PlantBot
            <div class="cb-fab-dot"></div>
        </div>"""
        fab_label = "🌿 PlantBot"
    else:
        pill_html = """<div id="cb-fab-marker"></div>
        <div class="cb-fab-pill" style="animation:none;background:linear-gradient(135deg,#444,#222);">
            <span style="font-size:15px;">✕</span> Close Chat
            <div class="cb-fab-dot"></div>
        </div>"""
        fab_label = "✕ Close Chat"
    ...
    if st.session_state.chatbot_open:
        # cb-panel-marker causes CSS to fix this block as the floating panel.
...
def main():
    ...
    # ── 3. Chatbot LAST — renders FAB + panel at bottom of DOM
    render_chatbot()
