...
# Hide sidebar entirely when chatbot is closed;
# show it (as floating panel) when open.
# ─────────────────────────────────────────────
def inject_chatbot_visibility():
...
def render_chatbot_sidebar():
    """Full-featured LeafLife Care chatbot rendered inside Streamlit sidebar."""
    ...
    /* ═══════════════════════════════════════════════
       ...
       The Streamlit sidebar is transformed into a
       fixed floating panel via CSS position tricks.
    ═══════════════════════════════════════════════ */

    /* Hide the sidebar rail / collapse arrow completely */
    ...
    /* Transform sidebar into a fixed bottom-right floating panel */
    [data-testid="stSidebar"] {
        ...
        box-shadow: 0 24px 80px rgba(0,0,0,0.22), 0 8px 32px rgba(0,0,0,0.12) !important;
    ...
    .leaflife-fab {
        ...
        box-shadow: 0 6px 24px rgba(233,30,140,0.50);
        display: flex;
        align-items: center;
        justify-content: center;
        cursor: pointer;
        ...
        font-size: 26px;
        animation: fabPop 2.5s infinite;
        pointer-events: none; /* visual; Streamlit button handles click */
    }
    @keyframes fabPop {
        0%,100% { box-shadow: 0 6px 24px rgba(233,30,140,0.50); transform: scale(1); }
        50%      { box-shadow: 0 10px 36px rgba(233,30,140,0.75), 0 0 0 10px rgba(233,30,140,0.08); transform: scale(1.04); }
    }
    .fab-dot {
    ...
    .cb-mode-card:hover { border-color: #E91E8C; background: #FFF0F5; transform: translateY(-2px); box-shadow: 0 4px 16px rgba(233,30,140,0.12); }
    ...
    .diag-result-card {
        ...
        margin: 12px 0; box-shadow: 0 4px 14px rgba(0,0,0,0.06);
    }
    .diag-result-header { padding: 12px 14px; border-bottom: 1px solid #F0F0F0; }
    .diag-result-body { padding: 12px 14px; }
    .care-step {
    ...
    /* ── Remedy card ── */
    .remedy-card {
        background: white; border: 1px solid #E8EDE8; border-radius: 10px;
    ...
    /* ── Quiz dots ── */
    .quiz-step-indicator { display: flex; gap: 6px; margin-bottom: 16px; }
    ...
    # ── FAB visual pill + hidden Streamlit button that toggles the sidebar ──
    # The CSS .leaflife-fab is pointer-events:none (visual only).
    # The actual st.button below is absolutely positioned over it via CSS.
    st.markdown("""
    <div class="leaflife-fab">
        🌿
        <div class="fab-dot"></div>
    </div>
    <style>
    /* Pin the FAB Streamlit button exactly over the visual FAB circle */
    div[data-testid="stVerticalBlock"]:has(button[data-testid="leaflife_fab_btn"]) {
        position: fixed !important;
    ...
    button[data-testid="leaflife_fab_btn"] {
        width: 62px !important;
    ...
    if st.button("🌿", key="leaflife_fab_btn", help="Open / close LeafLife Care chatbot"):
...
# ─────────────────────────────────────────────
def main():
    # ── 1. Control sidebar visibility via CSS ──
    inject_chatbot_visibility()
    ...
    # ── 4. FAB + sidebar chatbot (always rendered, CSS controls visibility) ──
