...
def render_navbar():
    ...
    pages = ["Home", "Plants", "Seeds", "Plant Care", "Offers", "About Us", "Contact", "Cart"]
    for i, (col, page) in enumerate(zip(cols[:8], pages)):
        with col:
...
# ─────────────────────────────────────────────
def render_chatbot():
    """Floating chatbot anchored to the bottom-right using Streamlit sidebar-like panel."""
    
    # Toggle button and window rendered in a fixed div via HTML+JS,
    # but the actual interactive logic lives in a st.sidebar-like expander
    # placed in a columns layout flush to the right.

    # We use a persistent st.empty at the bottom to hold the chatbot UI
    # without disrupting page layout.

    if not st.session_state.chatbot_open:
        # Render FAB button only
        fab_html = """
        <div style="position:fixed;bottom:28px;right:28px;z-index:9999;">
            ...
            <div style="
                ...
                white-space:nowrap;border:1px solid #C8E6C9;
                font-family:'DM Sans',sans-serif;
            ">PlantPulse Doctor</div>
        </div>
        """
    ...
    # FAB open button (always visible at bottom right)
    if not st.session_state.chatbot_open:
        col_spacer, col_fab = st.columns([9, 1])
        with col_fab:
            if st.button("PlantPulse", key="open_chatbot"):
                st.session_state.chatbot_open = True
                st.rerun()
    else:
        col_spacer, col_fab = st.columns([9, 1])
        with col_fab:
            if st.button("Close Chat", key="close_chatbot_fab"):
                st.session_state.chatbot_open = False
                st.session_state.chatbot_mode = None
                st.rerun()
...
# ─────────────────────────────────────────────
# CHATBOT FAB — always rendered at page bottom
# ─────────────────────────────────────────────
def render_chatbot_fab():
    """Renders the floating chatbot toggle at the very bottom of the page."""
    st.markdown("<div style='height:60px'></div>", unsafe_allow_html=True)
    cols = st.columns([8, 2])
    with cols[1]:
        label = "Close PlantPulse" if st.session_state.chatbot_open else "PlantPulse Doctor"
        btn_style = "background:#1B5E20;" if not st.session_state.chatbot_open else "background:#E53935;"
        st.markdown(f"""
        <style>
        ...
        """, unsafe_allow_html=True)
        if st.button(label, key="chatbot_fab_toggle", use_container_width=True):
...
def main():
    ...
    # Chatbot FAB always at the bottom
    render_chatbot_fab()

    # Chatbot window (opens in sidebar when active)
    render_chatbot()
