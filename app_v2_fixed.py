...
PRODUCTS = [
    ...
    # Air Purifying
    {
        ...
        "image": "https://images.unsplash.com/photo-1572461803616-bf57b63edb50?w=400&h=400&fit=crop",
...
def inject_css():
    ...
    * { box-sizing: border-box; }
    ...
    /* ── CHATBOT FLOATING PANEL (RIGHT SIDE) ── */
    .chatbot-fab {
        position: fixed;
        bottom: 28px;
        right: 28px;
    ...
    [data-testid="stSidebar"] {
        ...
        border-left: 2px solid #E8EDE8 !important;
    ...
    /* Hide Streamlit sidebar UI controls */
    ...
    [data-testid="stSidebar"] ::-webkit-scrollbar-thumb { background: #E8EDE8; border-radius: 2px; }
    ...
    .fab-open-btn-wrap button {
        ...
        box-shadow: 0 8px 28px rgba(233,30,140,0.5) !important;
        cursor: pointer !important;
        font-family: 'DM Sans', sans-serif !important;
        letter-spacing: 0.3px !important;
        transition: all 0.3s !important;
        animation: fabPulse2 2.8s infinite;
    }
    .fab-open-btn-wrap button:hover {
        transform: translateY(-3px) scale(1.04) !important;
        box-shadow: 0 14px 36px rgba(233,30,140,0.6) !important;
    }
    @keyframes fabPulse2 {
        0%,100% { box-shadow: 0 8px 28px rgba(233,30,140,0.5); }
        50%      { box-shadow: 0 8px 40px rgba(233,30,140,0.75), 0 0 0 10px rgba(233,30,140,0.08); }
    }
    </style>
...
def render_navbar():
    ...
    # Extra CSS for nav — single-line buttons, uniform height
    st.markdown("""
    <style>
    /* Force all nav buttons to single line, uniform height */
    ...
    /* Selectbox in nav — match button height */
    ...
    col_logo, col_nav, col_actions = st.columns([2, 7, 2])
    ...
    with col_nav:
        # 7 equal columns: Home, Plants, Seeds, Plant Care (dropdown), Offers, About, Contact
        ...
        with n4:
            # Plant Care as a compact selectbox-style dropdown
            care_choice = st.selectbox(
                "care_nav",
                ["Care ▾", "Fertilizers", "Pots & Planters", "Pest Control", "Soil & Compost", "Watering"],
                key="nav_care_select",
                label_visibility="collapsed",
            )
            if care_choice != "Care ▾":
                nav_to("category", selected_category=care_choice)
                st.rerun()
        with n5:
    ...
    st.markdown('<div style="height: 2px; background: linear-gradient(90deg, #2E7D32, #C8E6C9, #E91E8C, #C8E6C9, #2E7D32);"></div>', unsafe_allow_html=True)
...
def render_home():
    ...
    cat_images = [
        ...
        ("Flowering Plants", "https://images.unsplash.com/photo-1519378058457-4c29a0a2efac?w=300&h=400&fit=crop", "28+ varieties"),
...
def render_chatbot_panel():
    """Full-featured LeafLife Care chatbot rendered in RIGHT sidebar."""
    """Full-featured LeafLife Care chatbot rendered in right panel."""

    # ── Panel CSS overrides ──────────────────────────────────────────────
    st.markdown("""
    <style>
    /* Sidebar completely hidden — we use columns for chatbot */
    /* RIGHT-SIDE SIDEBAR — chatbot lives here */
    [data-testid="stSidebar"] {
        position: fixed !important;
        top: 0 !important;
        right: 0 !important;
    ...
    /* When sidebar is "collapsed" by Streamlit, still show it via our CSS */
    [data-testid="stSidebar"][aria-expanded="false"] {
    ...
    @keyframes fabPulse {
        ...
        50%      { box-shadow: 0 8px 40px rgba(233,30,140,0.70), 0 0 0 8px rgba(233,30,140,0.10); }
    }
    .fab-dot {
    ...
    .cb-mode-card:hover { border-color: #E91E8C; background: #FFF0F5; transform: translateY(-1px); }
    ...
    with st.sidebar:
        ...
        # ── Close button ────────────────────────────────────────────────────────
        if st.button("✕ Close Panel", key="close_chatbot_panel", use_container_width=True):
            st.session_state.chatbot_open = False
            st.rerun()
        ...
        if st.session_state.chatbot_mode is None:
            ...
            st.markdown("""
            <div style="margin-top:16px;padding:12px;background:#FFF0F5;border-radius:10px;border-left:3px solid #E91E8C;">
                <div style="font-size:12px;color:#E91E8C;font-weight:700;margin-bottom:4px;">Did you know?</div>
...
def main():
    # ── Always render chatbot in sidebar (RIGHT side via CSS) ──
    ...
    # ── FAB button: only show when chatbot is closed ──
    if not st.session_state.chatbot_open:
        st.markdown("""
        <style>
        /* Position the FAB container as fixed bottom-right */
        [data-testid="stVerticalBlock"]:has(> [data-testid="stVerticalBlock"] > [data-testid="element-container"] > .fab-anchor) {
            position: fixed !important;
        ...
        <div class="fab-anchor" style="display:none;"></div>
        """, unsafe_allow_html=True)

    render_page(page)

    # FAB to open chatbot — rendered at bottom of page
    if not st.session_state.chatbot_open:
        # Use a container at the very end; CSS will attempt to fix it bottom-right
        # The most reliable approach: inject a fixed HTML button via st.markdown
        st.markdown("""
        <div style="position:fixed;bottom:28px;right:28px;z-index:10000;">
            <div id="leaflife-fab-btn" onclick="
                // Find and click the hidden Streamlit button
                var btns = window.parent.document.querySelectorAll('button');
                for(var b of btns){ if(b.innerText.includes('Plant Care AI')){ b.click(); break; } }
            " style="
                background:linear-gradient(135deg,#E91E8C,#AD1457);
                color:white;
                padding:14px 24px;
            ...
            ">
                ...
                LeafLife Care
                <span style="width:8px;height:8px;background:#69F0AE;border-radius:50%;animation:dotBlink 1.4s infinite;display:inline-block;"></span>
            </div>
        </div>
        <style>
        @keyframes fabPulse2 {
        ...
        """, unsafe_allow_html=True)
        # Hidden Streamlit button that the JS click triggers
        if st.button("🌿 Plant Care AI", key="open_chatbot_fab",
                     help="Open LeafLife Care AI",
