...
def analyze_plant(image_file):
    """Dummy AI — returns a random diagnosis from DIAGNOSIS_MAP."""
    import random
    key = random.choice(list(DIAGNOSIS_MAP.keys()))
...
/* ═══════════════════════════════════════════════════════
   ...
   Streamlit sidebar is CSS-repositioned into a floating
   panel. All sidebar controls are hidden.
═══════════════════════════════════════════════════════ */

/* Hide Streamlit sidebar rail, collapse arrow, open button */
...
/* Reposition sidebar → bottom-right floating panel */
[data-testid="stSidebar"] {
    ...
    box-shadow: 0 32px 80px rgba(0,0,0,0.24), 0 8px 24px rgba(233,30,140,0.10) !important;
...
[data-testid="stSidebar"] > div:first-child::-webkit-scrollbar { width: 3px; }
...
.cb-fab-pill {
    ...
    pointer-events: none; /* visual only — real button sits on top */
...
/* Position the real Streamlit FAB button exactly over the visual pill */
div[data-testid="stVerticalBlockBorderWrapper"]:has(button[data-testid="cb_fab_toggle"]),
div[data-testid="element-container"]:has(button[data-testid="cb_fab_toggle"]) {
    position: fixed !important;
...
button[data-testid="cb_fab_toggle"] {
    opacity: 0 !important;
    position: fixed !important;
    bottom: 24px !important;
    right: 20px !important;
...
def render_chatbot():
    """
    Renders the complete chatbot — FAB toggle button + sidebar panel.
    Call this ONCE, before render_navbar(), so the sidebar is always mounted.
    """
    ...
    # ── FAB: visual pill (non-interactive, CSS animation) ───────────────────
    if not st.session_state.chatbot_open:
        fab_label = "🌿 PlantBot — Ask me anything"
        fab_icon  = "🌿"
    else:
        fab_label = "✕ Close PlantBot"
        fab_icon  = "✕"

    st.markdown(f"""
    <div class="cb-fab-pill">
        <span style="font-size:18px;">{fab_icon}</span>
        {"PlantBot" if not st.session_state.chatbot_open else "Close Chat"}
        <div class="cb-fab-dot"></div>
    </div>
    """, unsafe_allow_html=True)

    # ── FAB: real invisible Streamlit button layered over the pill ──────────
    if st.button(fab_label, key="cb_fab_toggle"):
        st.session_state.chatbot_open = not st.session_state.chatbot_open
        if st.session_state.chatbot_open:
            # Reset to welcome screen each time it opens fresh
            st.session_state.chatbot_mode     = None
            st.session_state.chatbot_messages = []
            st.session_state.diagnosis_result = None
    ...
    # ── Sidebar (hidden via CSS when closed) ────────────────────────────────
    # When chatbot_open=False we still render the sidebar so Streamlit keeps
    # it mounted; we just hide it with CSS opacity/visibility.
    if not st.session_state.chatbot_open:
        st.markdown("""
        <style>
        [data-testid="stSidebar"] {
        ...
        if uploaded:
            ...
            for kw, diag_key in key_map.items():
                if kw in sym_lower:
                    ...
                    break
            result = DIAGNOSIS_MAP.get(matched_key, list(DIAGNOSIS_MAP.values())[0])
...
# ─────────────────────────────────────────────────────────────────────────────
def main():
    # ── 1. Chatbot FIRST — must be rendered before any other st.columns/markdown
    #       so the sidebar is mounted from the top of the render tree.
    render_chatbot()
