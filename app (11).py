...
# ─────────────────────────────────────────────────────────────────────────────
# CHATBOT — RIGHT-SIDE FLOATING PANEL
# ─────────────────────────────────────────────────────────────────────────────
...
def _severity_badge(severity):
    ...
    return (
        ...
        f'border-radius:50px;font-size:11px;font-weight:700;">{severity} Severity</span>'
    )
...
def _confidence_bar(pct):
    ...
    <div style="margin:10px 0 4px;">
        <div style="display:flex;justify-content:space-between;margin-bottom:4px;">
            ...
            <span style="font-size:11px;color:#E91E8C;font-weight:700;">{pct}%</span>
        </div>
        <div style="background:#F0E0EC;border-radius:50px;height:6px;overflow:hidden;">
            <div style="width:{pct}%;height:100%;
                 background:linear-gradient(90deg,#E91E8C,#AD1457);
...
def analyze_plant(image_file):
    """Dummy AI — returns a random diagnosis from DIAG_MAP."""
    import random
    key = random.choice(list(DIAG_MAP.keys()))
    return DIAG_MAP[key]


# ─────────────────────────────────────────────────────────────────────────────
# CHATBOT — SIDEBAR FLOATING PANEL
# Uses Streamlit sidebar repositioned via CSS to bottom-right corner.
# The sidebar is ALWAYS mounted (initial_sidebar_state="expanded") and
# shown/hidden purely through CSS visibility toggling.
# ─────────────────────────────────────────────────────────────────────────────

CHATBOT_CSS = """
<style>
/* ── Completely hide the sidebar — we don't use it ── */
...
div[data-testid="element-container"]:has(> div[data-testid="stButton"] > button[kind="secondary"].cb-fab-btn) {
...
/* Style the FAB button itself */
button.cb-fab-btn,
div[data-testid="stButton"] button[kind="secondary"].cb-fab-btn {
    background: linear-gradient(135deg, #E91E8C 0%, #AD1457 100%) !important;
    ...
    box-shadow: 0 8px 28px rgba(233,30,140,0.50) !important;
    cursor: pointer !important;
    white-space: nowrap !important;
    animation: fabGlow 2.6s infinite !important;
    transition: transform 0.2s !important;
}
button.cb-fab-btn:hover {
    transform: scale(1.05) !important;
}

/* FAB close state */
button.cb-fab-btn-close,
div[data-testid="stButton"] button[kind="secondary"].cb-fab-btn-close {
...
@keyframes fabGlow {
    0%,100% { box-shadow: 0 8px 28px rgba(233,30,140,0.50); }
    50%      { box-shadow: 0 12px 40px rgba(233,30,140,0.80), 0 0 0 10px rgba(233,30,140,0.08); }
}
...
.cb-panel-wrap {
    ...
    background: #FAFAF7;
    border-radius: 20px;
    box-shadow: 0 24px 64px rgba(0,0,0,0.20), 0 8px 24px rgba(233,30,140,0.10);
...
.cb-title { font-family: 'Playfair Display', serif; font-size: 17px; font-weight: 700; color: white; }
.cb-subtitle { font-size: 11px; color: rgba(255,255,255,0.75); margin-top: 1px; }
...
.cb-panel-body::-webkit-scrollbar { width: 3px; }
.cb-panel-body::-webkit-scrollbar-thumb { background: #F48FB1; border-radius: 2px; }

/* ── Blinking dot ── */
.cb-fab-dot {
    width: 9px; height: 9px; background: #69F0AE;
    border-radius: 50%; animation: dotBlink 1.4s infinite; flex-shrink: 0;
}
@keyframes dotBlink { 0%,100%{opacity:1} 50%{opacity:0.3} }

/* ── Chat bubbles ── */
.chat-msg-bot {
    background: white; border: 1px solid #F0E6F0;
...
/* ── Mode cards ── */
.cb-mode-card {
    background: white; border: 2px solid #F0E6F0; border-radius: 14px;
    padding: 14px; margin-bottom: 10px; transition: all 0.22s; cursor: pointer;
}
.cb-mode-card:hover {
    border-color: #E91E8C; background: #FFF0F8;
    transform: translateY(-2px); box-shadow: 0 6px 20px rgba(233,30,140,0.12);
}
.cb-mode-icon { font-size: 26px; margin-bottom: 6px; }
.cb-mode-title { font-weight: 700; font-size: 14px; color: #1A1A1A; margin-bottom: 3px; }
.cb-mode-desc { font-size: 11px; color: #888; line-height: 1.5; }

/* ── Diagnosis result ── */
.diag-result-card {
    background: white; border-radius: 14px; border: 1px solid #F0E6F0;
    overflow: hidden; margin: 12px 0; box-shadow: 0 4px 16px rgba(0,0,0,0.06);
}
.diag-result-header { padding: 14px 16px; border-bottom: 1px solid #F5F0F5; }
.diag-result-body { padding: 12px 16px; }
.care-step {
    display: flex; gap: 10px; align-items: flex-start;
    padding: 7px 0; border-bottom: 1px solid #FAF0FA;
    font-size: 12px; color: #444; line-height: 1.6;
}
.care-step:last-child { border-bottom: none; }
.step-num {
    width: 20px; height: 20px; background: #FCE4EC; border-radius: 50%;
    display: flex; align-items: center; justify-content: center;
    font-size: 10px; font-weight: 700; color: #E91E8C; flex-shrink: 0; margin-top: 2px;
}
.remedy-card {
    background: white; border: 1px solid #F0E6F0; border-radius: 10px;
    padding: 10px; margin-bottom: 8px; display: flex; gap: 10px; align-items: center;
}
.remedy-img { width: 48px; height: 48px; border-radius: 8px; object-fit: cover; flex-shrink: 0; }

/* ── Suggestion quiz ── */
.quiz-dots { display: flex; gap: 6px; margin-bottom: 14px; }
.quiz-dot { height: 4px; border-radius: 2px; flex: 1; background: #F0E6F0; }
.quiz-dot.active { background: #E91E8C; }
.sugg-plant-card {
    background: white; border: 1px solid #F0E6F0; border-radius: 12px;
    overflow: hidden; margin-bottom: 10px; box-shadow: 0 2px 8px rgba(0,0,0,0.05);
}
.sugg-plant-img { width: 100%; height: 110px; object-fit: cover; }
.sugg-plant-body { padding: 10px 12px; }
.sugg-match-badge {
    display: inline-block; background: #FCE4EC; color: #E91E8C;
    padding: 2px 8px; border-radius: 50px; font-size: 10px; font-weight: 700; margin-bottom: 5px;
}

/* ── Chatbot-area Streamlit buttons (inside cb-panel scope) ── */
.cb-streamlit-zone .stButton > button[kind="primary"] {
    background: linear-gradient(135deg, #E91E8C, #AD1457) !important;
    border: none !important; color: white !important;
}
.cb-streamlit-zone .stButton > button[kind="primary"]:hover {
    background: linear-gradient(135deg, #AD1457, #880E4F) !important;
    transform: translateY(-1px) !important;
}
.cb-streamlit-zone .stButton > button {
    border-radius: 50px !important; font-family: 'DM Sans', sans-serif !important; font-weight: 600 !important;
}

/* ── Hide FAB button's own container gap ── */
div[data-testid="stMainBlockContainer"] > div > div > div:has(> div[data-testid="stVerticalBlock"] > div[data-testid="element-container"]:only-child > div[data-testid="stButton"] > button.cb-fab-btn) {
...
def render_chatbot():
    """
    Renders the chatbot as a CSS-fixed overlay panel.
    A single FAB button at bottom-right toggles open/close.
    No sidebar is used — zero layout disruption.
    """
    ...
    # ── FAB toggle button — the ONLY control ─────────────────────────────
    fab_label = "🌿 PlantBot" if not is_open else "✕ Close Chat"
    fab_extra_class = "cb-fab-btn-close" if is_open else ""

    # Inject class onto button via JS after render
    st.markdown(f"""
    <style>
    /* Target this specific button by its text */
    div[data-testid="stMainBlockContainer"] button[kind="secondary"]:has(> div > p:first-child) {{
        position: fixed !important;
        bottom: 24px !important;
        right: 24px !important;
        z-index: 10001 !important;
        background: {"linear-gradient(135deg, #555 0%, #333 100%)" if is_open else "linear-gradient(135deg, #E91E8C 0%, #AD1457 100%)"} !important;
        color: white !important;
        ...
        font-weight: 700 !important;
        font-size: 14px !important;
        box-shadow: {"0 6px 18px rgba(0,0,0,0.35)" if is_open else "0 8px 28px rgba(233,30,140,0.50)"} !important;
        cursor: pointer !important;
        white-space: nowrap !important;
        width: auto !important;
        min-width: 140px !important;
        {"" if is_open else "animation: fabGlow 2.6s infinite !important;"}
    }}
    /* Collapse the button's container so it takes zero layout space */
    div[data-testid="stMainBlockContainer"] div[data-testid="element-container"]:has(> div[data-testid="stButton"]) {{
        height: 0 !important;
    ...
    if st.button(fab_label, key="cb_fab_toggle"):
    ...
    # ── Chatbot panel (only rendered when open) ───────────────────────────
    if is_open:
...
def _render_chatbot_panel():
    ...
    Rendered as a fixed-position overlay div, completely outside normal flow."""

    # Inject the panel wrapper open tag + header
    st.markdown("""
        ...
        <div class="cb-header">
            <div style="display:flex;align-items:center;gap:12px;">
                ...
                <div style="display:flex;align-items:center;gap:5px;">
                    <div class="cb-fab-dot" style="position:static;width:8px;height:8px;"></div>
                    <span style="font-size:11px;color:rgba(255,255,255,0.65);">Live</span>
                </div>
            </div>
        </div>
    ...
    # NOTE: No "Close" button inside the panel — FAB is the only control.
    ...
    if st.session_state.chatbot_mode is None:
        ...
        <div class="cb-mode-card">
            ...
            <div class="cb-mode-desc">Upload a photo of your plant and describe symptoms. Get an AI diagnosis with treatment plan and product recommendations.</div>
        </div>
        """, unsafe_allow_html=True)
        if st.button("Start Diagnosis →", key="cb_btn_diag", type="primary", use_container_width=True):
            ...
            st.session_state.chatbot_messages = [{"role": "user", "text": "🌿 Plant Diagnosis"}]
            st.session_state.diagnosis_result = None
            st.rerun()
        ...
        <div class="cb-mode-card">
            ...
            <div class="cb-mode-desc">Answer 4 quick questions about your space & lifestyle and I'll suggest the perfect plants with prices and cart buttons.</div>
        </div>
        """, unsafe_allow_html=True)
        if st.button("Get Suggestions →", key="cb_btn_sugg", use_container_width=True):
            st.session_state.chatbot_mode     = "suggestions"
            st.session_state.chatbot_messages = [{"role": "user", "text": "🛒 Plant Buying Suggestions"}]
            st.session_state[SUGGESTION_RESULTS_KEY] = None
            st.rerun()

        st.markdown("""
        <div style="margin-top:14px;padding:12px;background:linear-gradient(135deg,#FFF0F8,#FCE4EC);
            ...
            <div style="font-size:11px;color:#555;line-height:1.6;">
                NASA's Clean Air Study found houseplants can remove up to 87% of indoor air toxins in 24 hours.
            </div>
        </div>
        """, unsafe_allow_html=True)
        # Close cb-panel-body and cb-panel-wrap
        st.markdown("</div></div>", unsafe_allow_html=True)
        return

    # ── BACK BUTTON (shown in all modes) ──────────────────────────────────
    if st.button("← Back to Menu", key="cb_back_top"):
        st.session_state.chatbot_mode     = None
        st.session_state.chatbot_messages = []
        st.session_state.diagnosis_result = None
        st.session_state[SUGGESTION_RESULTS_KEY] = None
    ...
    # ═══════════════════════════════════════════
    # MODE 1 — PLANT DIAGNOSIS
    # ═══════════════════════════════════════════
    if st.session_state.chatbot_mode == "diagnosis":
        handle_diagnosis()

    # ═══════════════════════════════════════════
    # MODE 2 — PLANT BUYING SUGGESTIONS
    # ═══════════════════════════════════════════
    elif st.session_state.chatbot_mode == "suggestions":
        handle_recommendations()

    # Close inner div, cb-panel-body, and cb-panel-wrap
...
def handle_diagnosis():
    ...
    <div style="padding:0 2px 10px;">
        <div style="font-size:11px;font-weight:700;text-transform:uppercase;
                    letter-spacing:1.5px;color:#E91E8C;margin-bottom:10px;">
            🔬 Plant Diagnosis Doctor
        </div>
        <div class="chat-msg-bot">
            Please upload a photo of your plant and describe what you're seeing.
    ...
    uploaded = st.file_uploader(
        ...
        help="Upload a clear photo of the affected plant or leaf",
    )
    ...
    # Quick symptom chips
    st.markdown("""
    <div style="font-size:11px;color:#888;margin-bottom:6px;">Quick select:</div>
    """, unsafe_allow_html=True)
    chip_cols = st.columns(2)
    quick = ["Yellow leaves", "Brown tips", "Wilting", "Spots/fungus",
             "Root rot smell", "Leggy growth", "Drooping", "Pale color"]
    ...
    if st.button(
        ...
        disabled=not diag_ready,
    ):
        if uploaded:
            result = analyze_plant(uploaded)
        else:
            # Map symptom keywords to diagnosis keys
            ...
            key_map = {
                ...
                "yellow and pale": "Nutrient Deficiency",
                "root": "Root Rot",
                "rot": "Root Rot",
            ...
            matched_key = None
            for kw, diag_key in key_map.items():
                if kw in sym_lower:
                    matched_key = diag_key
                    break
            diag_key_remap = {
                "Overwatering": "Yellowing leaves",
                "Underwatering": "Wilting / Drooping",
                "Fungal Leaf Spot": "Spots on leaves",
                "Spider Mite Infestation": "Sticky residue on leaves",
                "Nutrient Deficiency": "Pale / washed-out color",
                "Root Rot": "Wilting / Drooping",
                "Insufficient Light": "No new growth",
                "Sunburn / Heat Stress": "Pale / washed-out color",
            }
            matched_key = diag_key_remap.get(matched_key, matched_key)
            result = DIAG_MAP.get(matched_key, list(DIAG_MAP.values())[0])
    ...
    if st.session_state.diagnosis_result:
        ...
        <div class="chat-msg-bot" style="margin-top:10px;">
            ...
            I've analysed your plant and here's what I found:
        </div>

        <div class="diag-result-card">
            <div class="diag-result-header" style="background:linear-gradient(135deg,#FFF0F8,#FCE4EC);">
                <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px;">
                    <span style="font-size:24px;">{result.get('icon','🌿')}</span>
                    <div>
                        <div style="font-family:'Playfair Display',serif;font-size:16px;
                                    font-weight:700;color:#1A1A1A;">{result.get('condition', result.get('issue',''))}</div>
                ...
                <div style="font-size:12px;color:#555;margin-top:8px;line-height:1.6;">
                    {result.get('cause', result.get('description',''))}
                </div>
            </div>
            <div class="diag-result-body">
                <div style="font-size:11px;font-weight:700;text-transform:uppercase;
                            letter-spacing:1px;color:#E91E8C;margin-bottom:10px;">
                    💊 Treatment Steps
                </div>
        """, unsafe_allow_html=True)

        for i, step in enumerate(result.get("care_steps", result.get("steps", [])), 1):
            st.markdown(f"""
            <div class="care-step">
        ...
        if result.get("care_schedule"):
            st.markdown("""
            <div style="background:#FFF0F8;border-radius:12px;padding:12px 14px;margin:10px 0;">
                <div style="font-size:11px;font-weight:700;text-transform:uppercase;
            ...
            for day, task in result["care_schedule"].items():
                ...
                <div style="display:flex;gap:8px;align-items:flex-start;
                            padding:5px 0;border-bottom:1px solid #F8E8F0;
                            font-size:12px;color:#444;">
                    <span style="font-weight:700;color:#E91E8C;min-width:72px;">{day}</span>
                    <span>{task}</span>
                </div>
                """, unsafe_allow_html=True)
            st.markdown("</div>", unsafe_allow_html=True)
        ...
        remedy_products = [
            p for p in PRODUCTS
            if p["name"] in result.get("remedy_products", [])
        ]
        if remedy_products:
            st.markdown("""
            <div style="font-size:11px;font-weight:700;text-transform:uppercase;
            ...
            for prod in remedy_products:
                ...
                """, unsafe_allow_html=True)
                if st.button(f"🛒 Add to Cart", key=f"diag_atc_{prod['id']}", use_container_width=True, type="primary"):
                    add_to_cart(prod["id"])
...
def handle_recommendations():
    ...
    # Progress dots
    step_num = 0 if not results_ready else 4
    dots_html = '<div class="quiz-dots">'
    for i in range(4):
        cls = "quiz-dot active" if i < step_num or results_ready else "quiz-dot"
    ...
    if not results_ready:
        ...
        location = st.radio(
            "Location",
            ["🏠 Indoor (inside your home/office)", "🌳 Outdoor (balcony/garden/terrace)", "🏡 Both Indoor & Outdoor"],
            key="q_location",
            label_visibility="collapsed",
        )
        ...
        light = st.radio(
            "Light",
            ["🌑 Very Low (no direct sun)", "🌤 Indirect Bright (filtered light)", "⛅ Partial Sun (morning sun)", "☀️ Full Sun (6+ hrs direct)"],
            key="q_light",
            label_visibility="collapsed",
        )

        st.markdown("**Q3 — How much time for plant care?**")
        maintenance = st.radio(
            "Maintenance",
            ["😴 Minimal — water once a week or less", "🌿 Moderate — water 2–3×/week", "🌸 Enthusiast — daily attention"],
            key="q_maintenance",
            label_visibility="collapsed",
        )
        ...
        purpose = st.multiselect(
            "Purpose",
            ["Air Purification", "Flowering / Colour", "Gifting", "Low Maintenance",
             "Aesthetic Decor", "Pet Safe", "Beginner Friendly"],
            key="q_purpose",
            label_visibility="collapsed",
        ...
        st.markdown(f"""
        <div class="chat-msg-bot">
            🎉 Found <strong>{len(results)}</strong> perfect plant matches for you!
            Tap any <em>Add to Cart</em> to get started.
        </div>
        """, unsafe_allow_html=True)
        ...
        for rank, prod in enumerate(results):
            ...
            disc      = discount_pct(prod["price"], prod["original_price"])
            care_map  = {"Very Easy": "🟢 Easy care", "Easy": "🟡 Some care", "Moderate": "🟠 Moderate", "Expert": "🔴 Advanced"}
            air_tag   = " · 💨 Air Purifier" if prod.get("air_purifying") else ""

            st.markdown(f"""
            <div class="sugg-plant-card">
                <div style="position:relative;">
                    <img class="sugg-plant-img"
                         src="{prod['image']}"
                         alt="{prod['name']}"
                ...
                <div class="sugg-plant-body">
                    ...
                    <div style="font-size:11px;color:#777;margin-bottom:7px;">
                        {care_map.get(prod.get('care_level',''), '🟡 Some care')}{air_tag}
                    </div>
                    <div style="display:flex;align-items:center;justify-content:space-between;">
                        <div>
                            <span style="font-weight:700;font-size:16px;color:#1B5E20;">₹{prod['price']}</span>
                            <span style="font-size:11px;color:#bbb;text-decoration:line-through;margin-left:4px;">₹{prod['original_price']}</span>
                        </div>
                        <div style="color:#FFA000;font-size:12px;">{"★" * int(prod['rating'])} {prod['rating']}</div>
                    </div>
                </div>
            </div>
            """, unsafe_allow_html=True)
            ...
            with sc1:
                if st.button("🛒 Add to Cart", key=f"sugg_atc_{prod['id']}", use_container_width=True, type="primary"):
                    add_to_cart(prod["id"])
...
# ─────────────────────────────────────────────────────────────────────────────
def main():
    # ── 1. Chatbot FIRST — sidebar must be mounted before main content renders
    render_chatbot()
