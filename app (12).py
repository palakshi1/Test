...
.cb-mode-card {
    ...
    padding: 14px; margin-bottom: 10px; transition: all 0.22s;
}
.cb-mode-card:hover { border-color: #E91E8C; background: #FFF0F8; }
...
/* Style primary buttons inside chatbot panel */
.cb-panel-wrap .stButton > button[kind="primary"] {
    background: linear-gradient(135deg, #E91E8C, #AD1457) !important;
    border: none !important; color: white !important;
}
.cb-panel-wrap .stButton > button {
    border-radius: 50px !important; font-family: 'DM Sans', sans-serif !important; font-weight: 600 !important;
}
</style>
"""

# FAB HTML — rendered via components.html so it's truly fixed-position, outside Streamlit's DOM
FAB_HTML_OPEN = """
<style>
  .cb-fab {
  ...
  .cb-fab:hover { transform: scale(1.06); box-shadow: 0 12px 36px rgba(233,30,140,0.70); }
  .cb-fab .dot { width: 9px; height: 9px; background: #69F0AE; border-radius: 50%;
...
<button class="cb-fab" onclick="window.parent.location.href=window.parent.location.href.split('?')[0]+'?cb=1'">
  <span>🌿</span> PlantBot <div class="dot"></div>
</button>
"""
...
<style>
  .cb-fab-close {
    ...
    color: white; border: 3px solid rgba(255,255,255,0.25); border-radius: 50px;
    padding: 15px 24px; font-size: 15px; font-weight: 700;
    font-family: 'DM Sans', sans-serif;
  ...
  .cb-fab-close:hover { transform: scale(1.05); }
</style>
<button class="cb-fab-close" onclick="window.parent.location.href=window.parent.location.href.split('?')[0]">
...
def render_chatbot():
    """
    FAB button: rendered via components.html (truly fixed, no layout disruption).
    Panel: rendered inline with st.markdown (position:fixed CSS panel).
    Toggle: driven by ?cb=1 query param — no sidebar used at all.
    """
    # ── Session defaults ──────────────────────────────────────────────────
    for key, default in [
        ("chatbot_open", False),
    ...
    # ── Read toggle from query param ──────────────────────────────────────
    params = st.query_params
    if "cb" in params and params["cb"] == "1":
        st.session_state.chatbot_open = True
    elif "cb" not in params:
        st.session_state.chatbot_open = False

    is_open = st.session_state.chatbot_open

    # ── Inject panel CSS (zero layout cost) ──────────────────────────────
    st.markdown(CHATBOT_PANEL_CSS, unsafe_allow_html=True)

    # ── FAB button via components.html — truly fixed, outside Streamlit DOM ──
    components.html(
        FAB_HTML_CLOSE if is_open else FAB_HTML_OPEN,
        height=0,   # zero height — button is fixed-position, needs no space
    )
...
def _render_chatbot_panel():
    """Chatbot panel rendered as a CSS-fixed overlay. No sidebar. No layout shift."""
