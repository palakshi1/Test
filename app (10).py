...
CHATBOT_CSS = """
<style>
/* ── Hide all default sidebar chrome ── */
[data-testid="stSidebarCollapseButton"],
[data-testid="collapsedControl"],
section[data-testid="stSidebar"] > div > button { display: none !important; }

/* ── Reposition sidebar as bottom-right floating panel ── */
section[data-testid="stSidebar"] {
    ...
    box-shadow: 0 24px 64px rgba(0,0,0,0.20), 0 8px 24px rgba(233,30,140,0.10) !important;
...
section[data-testid="stSidebar"] > div:first-child::-webkit-scrollbar { width: 3px; }
section[data-testid="stSidebar"] > div:first-child::-webkit-scrollbar-thumb {
    background: #F48FB1; border-radius: 2px;
}

/* ── FAB pill (visual only, pointer-events off) ── */
.cb-fab-pill {
    position: fixed;
    bottom: 24px;
    right: 20px;
    ...
    font-weight: 700;
    font-size: 14px;
    box-shadow: 0 8px 28px rgba(233,30,140,0.50);
...
/* ── Real FAB button: fixed over pill, full opacity 0 ── */
div[data-testid="stMainBlockContainer"] div.cb-fab-btn-wrap {
...
def render_chatbot():
    """
    Renders the FAB toggle button in the main page, and the chatbot panel
    inside st.sidebar (which is CSS-repositioned to bottom-right corner).
    The sidebar is always mounted; shown/hidden via CSS opacity/pointer-events.
    """
    ...
    # ── Visual FAB pill (non-interactive decoration) ──────────────────────
    if not is_open:
        st.markdown('''
        <div class="cb-fab-pill">
            <span style="font-size:18px;">🌿</span>
            PlantBot
            <div class="cb-fab-dot"></div>
        </div>''', unsafe_allow_html=True)
    else:
        st.markdown('''
        <div class="cb-fab-pill cb-fab-pill-close">
            <span style="font-size:16px;">✕</span>
            Close Chat
        </div>''', unsafe_allow_html=True)

    # ── Real Streamlit FAB button (transparent, positioned over pill) ─────
    fab_css = """
    <style>
    /* Find the button container by targeting the element just before it */
    div[data-testid="stMainBlockContainer"] div[data-testid="element-container"]:has(button[data-testid="baseButton-secondary"][aria-label="🌿 PlantBot"]),
    div[data-testid="stMainBlockContainer"] div[data-testid="element-container"]:has(button[data-testid="baseButton-secondary"][aria-label="✕ Close Chat"]) {
        position: fixed !important; bottom: 24px !important; right: 20px !important;
        z-index: 10001 !important; width: 160px !important;
    }
    div[data-testid="stMainBlockContainer"] div[data-testid="element-container"]:has(button[aria-label="🌿 PlantBot"]) button,
    div[data-testid="stMainBlockContainer"] div[data-testid="element-container"]:has(button[aria-label="✕ Close Chat"]) button {
        opacity: 0 !important; width: 160px !important; height: 52px !important;
        cursor: pointer !important; border-radius: 50px !important;
    }
    </style>
    """
    ...
    # ── Sidebar panel (always mounted, visibility controlled by CSS) ──────
    with st.sidebar:
...
def _render_chatbot_panel():
    ...
    # ── CLOSE BUTTON ──────────────────────────────────────────────────────
    if st.button("✕ Close", key="cb_close_btn", use_container_width=True):
        st.session_state.chatbot_open = False
        st.rerun()
